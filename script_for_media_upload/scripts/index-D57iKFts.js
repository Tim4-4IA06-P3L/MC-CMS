...
const getMetadata = (file2) => {
  if (!file2.filepath) {
    return new Promise((resolve, reject) => {
      const pipeline = sharp__default.default({ failOnError: false });
      pipeline.metadata().then(resolve).catch(reject);
      file2.getStream().pipe(pipeline);
    });
  }
  return sharp__default.default(file2.filepath, { failOnError: false }).metadata();
};
...
const resizeFileTo = async (file2, options, {
  ...
  let newInfo;
  if (!file2.filepath) {
    const transform = sharp__default.default({ failOnError: false }).resize(options).on("info", (info) => {
      newInfo = info;
    });
    await writeStreamToFile(file2.getStream().pipe(transform), filePath);
  } else {
    newInfo = await sharp__default.default(file2.filepath, { failOnError: false }).resize(options).toFile(filePath);
  }
...
const optimize = async (file2) => {
  ...
  if ((sizeOptimization || autoOrientation) && isOptimizableFormat(format)) {
    let transformer;
    if (!file2.filepath) {
      transformer = sharp__default.default({ failOnError: false });
    } else {
      transformer = sharp__default.default(file2.filepath, { failOnError: false });
    }
...
const isFaultyImage = async (file2) => {
  ...
  try {
    await sharp__default.default(file2.filepath, { failOnError: false }).stats();
    return false;
  } catch (e) {
    return true;
  }
